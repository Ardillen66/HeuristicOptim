...
std::vector<int> buildACOSolution(std::vector<int> & curBest, std::vector< std::vector<long double> > & pherTrails){
  ...
  for (int i = 1; i < solSize; ++i)
  {
    ...
    if (u <= 0.4)
    {
      ...
      for (int j = 0; j < 5 && jobsRemaining; ++j)
        {
          int unschedIndex = getUnscheduled(selectedScheduled);
          //cout << "Resulting index: " << unschedIndex << endl;
          if(unschedIndex > 0 ){
...
void updatePheromoneTrails(std::vector<int> & curBest, std::vector<int> & newBest, long int newWCT, 
  ...
  //cout << "Update pheromone trails" << endl;
  for (int i = 1; i < solSize; ++i) //Positions in new best solution
  {
    int job = newBest[i];
    //cout << "Update trail for job " << job << endl;
    ...
    {
      if (j == i)
        pherTrails[j-1][jobIndex] = trailPersistence * pherTrails[j-1][jobIndex] + (10000.0 / (diff * (1.0 * newWCT)));
      else
        pherTrails[j-1][jobIndex] = trailPersistence * pherTrails[j-1][jobIndex];
    }
  }
}